...
- `deploy/socratic-tutor.service` — systemd unit template, `${APP_USER}` and `${APP_DIR}` substituted. Single worker (mandatory; in-memory sessions).
- `deploy/socratic-tutor.env.template` — backend env template; rendered to `/etc/socratic-tutor.env` with `chmod 600`.
...
- `deploy/deploy.sh` — every-deploy, run as `APP_USER`. `git pull` → backend `pip install` → frontend `npm ci && npm run build` → `rsync` to `/var/www/socratic-tutor/` → restart backend → reload nginx → smoke test `/health` + `/specializations`.
...
# clone repo to $APP_DIR (default /opt/socratic-tutor/app), then:
cp deploy/deploy.env.example deploy/deploy.env  # edit APP_HOSTNAME + ADMIN_EMAIL
sudo bash deploy/bootstrap.sh
sudo nano /etc/socratic-tutor.env               # replace replace-me with the real OPENROUTER_API_KEY
...
sudo systemctl enable socratic-tutor             # survive reboots
```
